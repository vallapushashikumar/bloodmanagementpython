...
class main_page(tk.Frame):
    def __init__(self):
        ...
        self.text1 = tk.Text(self,  height=2, width=45, bg='cyan')

        self.text1.grid(row=4, column=7)

        self.button_1 = tk.Button(self, text='Register here ', bg='green', command=self.register)
        self.button_1.grid(row=7, column=7)
        self.button_2 = tk.Button(self, text='nearest center info', bg='green', width=50, command=self.get_info)
        self.button_2.grid(row=9, column=7)
        self.button_3 = tk.Button(self, text='Request blood group', bg='green', command=abus.about_us_display)
    ...
    def register(self):
        ...
        self.month = ttk.Combobox(self.win,
    ...
    def print_data(self):
        ...
        self.printing = c.execute(
            'SELECT ID, NAME, GENDER, BLOODGROUP, PHONE, CITY, DOB FROM REG R JOIN COMPAT C WHERE C.RBGRP=? AND R.BLOODGROUP=C.DBGRP AND CITY=?;',
            (self.recv_bg, self.recv_city,))
        count = 0
        row1 = ()
